<template>
    <form method="post" :action="postAction" @submit.prevent>
        <div class="input-group mb-3">
            <input type="text" class="form-control" name="name" v-model="form.name" placeholder="name" aria-label="Username" aria-describedby="basic-addon1">
        </div>

        <div class="input-group mb-3">
            <input type="email" class="form-control" v-model="form.email" placeholder="email" aria-label="Recipient's username" aria-describedby="basic-addon2">
        </div>

        <label for="basic-url" class="form-label">Is this an employee</label>
        <div class="input-group mb-3">
            <select v-model="form.is_employee" class="form-select" aria-label="Default select example">
                <option v-if="form.is_employee==0" selected value="0">No</option>
                <option value="1">Yes</option>
            </select>
        </div>

        <label for="basic-url" class="form-label">Company</label>
        <div class="input-group mb-3">
            <select v-model="form.company_id" class="form-select" aria-label="Default select example">
                <option selected>{{user.company ? user.company.name : 'none'}}</option>
                <option v-for="(company, index) in companies" :key="index" :value="company.id">{{company.name}}</option>
            </select>
        </div>
        <div class="input-group mb-3">
            <input type="submit" value="submit" class="btn btn-primary" @click="submit()">
        </div>
        
    </form>
</template>

<script>
//import Input from '../../../vendor/laravel/breeze/stubs/inertia/resources/js/Components/Input.vue';
export default {
  //components: { Input },
    name:'EditUserComponent',
    props:{
        panelTitle:{
            default:'Edit',
            type:String,
            required:true
        },
        postAction:{
            type:String,
            required:true
        },
        userId:{
            type:String,
            required: true
        }
        
    },
    data(){
        return{
            user:{},
            companies:[],
            form:{
                email:'',
                name:'',
                company_id:'',
                is_employee:'0'
            },

        }
    },

    computed:{

    },

    created(){
        axios.get('/companies')
        .then(res=>{
            if (res.data.success) {
                this.companies = res.data.companies
            }else{
                 toastr.error(res.data.message);
            }
        })
        .catch(err => {
                toastr.error("An error occured, please try again")
                console.log(err);
        });

        //get user
        axios.get('/admin/user/'+this.userId)
        .then(res=>{
            if (res.data.success) {
                this.user = res.data.user
                this.form.email = this.user.email
                this.form.name = this.user.name
                this.form.company_id = this.user.company_id
                this.form.is_employee = this.user.is_employee
            }else{
                 toastr.error(res.data.message);
            }
        })
        .catch(err => {
                toastr.error("An error occured, please try again")
                console.log(err);
        });
    },

    methods: {
        submit(){
            axios.patch(this.postAction, this.form)
            .then(res => {
                if (res.data.success) {
                    toastr.success(res.data.message);
                } else {
                    toastr.warning(res.data.message);
                }
            })
            .catch(err => {
                if(err.response && err.response.status === 422){
                    //alert('ki')
                    toastr.error(err.response.data.message)
                }else{
                    toastr.error("An error occured, please try again")
                    console.log(err);
                }
            });
        }
    }


}
</script>